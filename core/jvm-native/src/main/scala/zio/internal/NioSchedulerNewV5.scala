...
private final class NioScheduler(autoBlocking: Boolean) extends Executor { parent =>
    ...
    private[this] val loadingState    = new AtomicInteger(0)
    ...
    private def maybeUnparkWorker(currentState: Int): Unit = {
        ...
        if (currentActive != poolSize && currentSearching == 0) {
            ...
            if (worker ne null) {
                ...
                workersActiveTracker.unfreezeWorker(worker)
            }
        }
    }
                ...
                while (!isInterrupted) {
                    ...
                    if (runnable eq null) {
                        ...
                        while (!active && !isInterrupted) {
                            LockSupport.park()
                            workerTracker.freezeWorker(worker)
                        }
...
private object NioScheduler {
    ...
    private final class WorkerActivityTracker(maxSize: Int) {
        ...
        def freezeWorker(worker: NioScheduler.Worker): Unit = synchronized {
            if (dummyHead.next == dummyTail) null
            else {
                val node = nodeMap.get(worker)
                if (node ne null) {
                    removeNode(node)
                }
            }
        ...
        def unfreezeWorker(worker: NioScheduler.Worker): Unit = synchronized {
            if (dummyHead.next == dummyTail) null
            else {
                val node = nodeMap.get(worker)
                if (node ne null) {
