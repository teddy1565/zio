...
private object NioScheduler {
    ...
    private final class WorkerActivityTracker(maxSize: Int) {
        ...
        private val workerIdMapKey = new java.util.concurrent.ConcurrentHashMap[Int, NioScheduler.Worker]()
        private val workerIdMapVal = new java.util.concurrent.ConcurrentHashMap[NioScheduler.Worker, Int]()

        (0 until maxSize).foreach { workerId =>
            workerIdList(maxSize) = workerId
        }
        
        def addWorkerToIdList(worker: NioScheduler.Worker, id: Int): Unit = synchronized {
            workerIdMapKey.put(id, worker)
            workerIdMapVal.put(worker, id)
        }

        def replaceWorkerIdList(oldWorker: NioScheduler.Worker, newWorker: NioScheduler.Worker): Unit = synchronized {
